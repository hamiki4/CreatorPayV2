...
public sealed class NotificationService(ApplicationDbContext db, IOptions<WalletOptions> walletConfigured) : INotificationService
{
    ...
    async Task AddLowBalanceNotificationAsync(Guid purchaseId, DateTime now, CancellationToken ct)
    {
        ...
        if (entry is null || entry.BalanceBefore < walletOptions.LowBalanceThreshold || entry.BalanceAfter >= walletOptions.LowBalanceThreshold) return;
        ...
        db.Notifications.Add(n); db.NotificationOutboxMessages.Add(new NotificationOutboxMessage { Id = Guid.NewGuid(), NotificationId = n.Id, Status = NotificationOutboxStatus.Pending, AvailableAtUtc = now, CreatedAtUtc = now }); Audit(n.Id, "NotificationCreated", null);
    }
