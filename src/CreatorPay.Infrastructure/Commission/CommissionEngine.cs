...
{
 public async Task<SelectedCommissionRule>SelectAsync(CommissionSelectionRequest r,CancellationToken ct){if(r.CalculationAtUtc.Kind!=DateTimeKind.Utc)throw new CommissionConfigurationException("CalculationAtUtc must be UTC.");var currency=r.CurrencyCode.Trim().ToUpperInvariant();
