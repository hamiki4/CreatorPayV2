...
public sealed class MerchantStore(ApplicationDbContext db) : IMerchantStore
{
    ...
    public async Task<(UserAccount User,Merchant Merchant)?> FindByTokenAsync(string hash,string purpose,CancellationToken ct){var row=await(from t in db.MerchantVerificationTokens join u in db.UserAccounts on t.UserAccountId equals u.Id join m in db.Merchants on u.MerchantId equals m.Id where t.TokenHash==hash&&t.Purpose==purpose select new{u,m}).SingleOrDefaultAsync(ct);return row is null?null:(row.u,row.m);} public Task<MerchantVerificationToken?> FindTokenAsync(string h,string p,CancellationToken ct)=>db.MerchantVerificationTokens.SingleOrDefaultAsync(x=>x.TokenHash==h&&x.Purpose==p,ct);
