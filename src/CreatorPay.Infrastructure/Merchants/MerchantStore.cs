...
{
    public Task<bool> EmailExistsAsync(string v, Guid? e, CancellationToken ct) => db.UserAccounts.AnyAsync(x => x.NormalizedEmail == v && (!e.HasValue || x.Id != e), ct); public async Task<bool> PhoneExistsAsync(string v, Guid? e, CancellationToken ct) => await db.Merchants.AnyAsync(x => x.NormalizedPhoneNumber == v && (!e.HasValue || x.Id != e), ct) || await db.Creators.AnyAsync(x => x.NormalizedPhoneNumber == v, ct) || await db.Customers.AnyAsync(x => x.NormalizedPhoneNumber == v, ct);
