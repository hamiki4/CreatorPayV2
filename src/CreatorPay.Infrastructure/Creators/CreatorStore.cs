...
public sealed class CreatorStore(ApplicationDbContext db) : ICreatorStore
{
    ...
    public Task<Creator?> FindCreatorAsync(Guid id, CancellationToken ct) => db.Creators.SingleOrDefaultAsync(x => x.Id == id, ct);
    public async Task<(UserAccount User, Creator Creator)?> FindByUserAsync(Guid id, CancellationToken ct)
    {
        var row = await (from u in db.UserAccounts join c in db.Creators on u.CreatorId equals c.Id where u.Id == id select new { u, c }).SingleOrDefaultAsync(ct);
