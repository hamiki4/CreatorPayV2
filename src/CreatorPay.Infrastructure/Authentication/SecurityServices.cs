...
public sealed class TokenService(IOptions<JwtOptions> options, IUtcClock clock) : ITokenService
{
    public (string Token, DateTime ExpiresAtUtc) CreateAccessToken(UserAccount user)
    {
        ...
        var claims = new List<Claim> { new(JwtRegisteredClaimNames.Sub, user.Id.ToString()), new(ClaimTypes.NameIdentifier, user.Id.ToString()), new(ClaimTypes.Role, user.Role.ToString()), new(JwtRegisteredClaimNames.Email, user.Email), new(JwtRegisteredClaimNames.Jti, Guid.NewGuid().ToString("N")), new(JwtRegisteredClaimNames.Iat, EpochTime.GetIntDate(now).ToString(), ClaimValueTypes.Integer64) };
        Add(claims, "creator_id", user.CreatorId); Add(claims, "merchant_id", user.MerchantId); Add(claims, "supervisor_id", user.SupervisorId); Add(claims, "cashier_id", user.CashierId);
