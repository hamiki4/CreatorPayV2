...
public sealed class WalletService(ApplicationDbContext db, IOptions<WalletOptions> options) : IWalletService
{
    ...
    public async Task<PurchaseDto> GetPurchaseAsync(Guid id, Guid? merchantId, Guid? cashierId, CancellationToken ct)
    {
        var p = await Query().SingleOrDefaultAsync(x => x.Id == id && (!merchantId.HasValue || x.MerchantId == merchantId) && (!cashierId.HasValue || x.CashierId == cashierId), ct) ?? throw new KeyNotFoundException();
        var creator = await db.Creators.AsNoTracking().SingleAsync(x => x.Id == p.CreatorId, ct);
        return Map(p, p.CommissionSnapshot, creator.DisplayName, CreatorPublicId(creator), p.MerchantLocationId?.ToString() ?? "Not assigned", null);
    }
    public async Task<IReadOnlyList<PurchaseDto>> GetPurchasesAsync(Guid? merchantId, Guid? cashierId, int take, CancellationToken ct)
    {
        var q = Query();
        if (merchantId.HasValue) q = q.Where(x => x.MerchantId == merchantId);
        if (cashierId.HasValue) q = q.Where(x => x.CashierId == cashierId);
        var purchases = await q.OrderByDescending(x => x.TransactionDateUtc).Take(Math.Clamp(take, 1, 100)).ToListAsync(ct);
        var creatorIds = purchases.Select(x => x.CreatorId).Distinct().ToArray();
        ...
        return purchases.Select(x =>
        {
            ...
            return Map(x, x.CommissionSnapshot, creator.DisplayName, CreatorPublicId(creator), x.MerchantLocationId?.ToString() ?? "Not assigned", null);
        }).ToList();
    }
