...
public sealed class WalletService(ApplicationDbContext db, IOptions<WalletOptions> options) : IWalletService
{
    ...
    public async Task<ConfirmedSalesReportDto> GetConfirmedSalesAsync(Guid merchantId, ConfirmedSalesQuery request, CancellationToken ct)
    {
        ...
        var locationIds = all.Select(x => x.MerchantLocationId).Distinct().ToArray();
    ...
    private async Task<MerchantWallet> EnsureWallet(Guid merchantId, CancellationToken ct)
    {
        ...
        var activeStatus = MerchantWalletStatus.Active.ToString();
        var inserted = await db.Database.ExecuteSqlInterpolatedAsync($"""
            INSERT INTO "MerchantWallets" ("Id", "MerchantId", "CurrencyCode", "AvailableBalance", "HeldBalance", "Status", "CreatedAtUtc")
            VALUES ({walletId}, {merchantId}, {settings.CurrencyCode}, 0, 0, {activeStatus}, {now})
            ON CONFLICT ("MerchantId", "CurrencyCode") DO NOTHING
            """, ct);
        if (inserted == 1)
        {
        ...
        return await db.MerchantWallets.SingleAsync(x => x.MerchantId == merchantId && x.CurrencyCode == settings.CurrencyCode, ct);
    }
