...
test('admin operations exposes the simplified secure pilot navigation',async()=>{
  ...
  for(const item of ['Dashboard','Reports','Creator Review','Business Review','Admin Accounts','Business Accounts','Creator Accounts','Customer Accounts','Cashier Accounts','Commission','Deposits','Wallets','Payouts','Fraud','System','Remove Account','Login Access']) assert.match(source,new RegExp(item))
  ...
  assert.match(source,/OperationsAdmin/)
  ...
  for(const cashierField of ['cashierName','businessName','publicBusinessId','assignedLocation','Password Reset Requests']) assert.match(source,new RegExp(cashierField))
  assert.match(source,/headerFor\(value: string\)/)
  assert.match(source,/Disable Cashier/)
  assert.match(api,/MerchantCashiers/)
  for(const accountFilter of ['General Search','All statuses','Business name or public ID','Clear Filters','Admin Accounts','Business Accounts','Creator Accounts','Customer Accounts','Cashier Accounts']) assert.match(source,new RegExp(accountFilter))
