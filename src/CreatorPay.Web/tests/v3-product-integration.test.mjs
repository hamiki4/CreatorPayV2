...
const business=read('BusinessDashboard.tsx')
const promotions=read('PromotionWorkspace.tsx')
const ugc=read('UgcWorkspace.tsx')
const customer=read('CustomerWorkspace.tsx')
const cashier=read('CashierCheckoutWorkspace.tsx')
const wallet=read('WalletWorkspace.tsx')
const financial=read('ProductAdminWorkspaces.tsx')
...
test('consolidated Business Creator Customer and Cashier surfaces expose the final product model',()=>{
  for(const label of ['Home','Promotions','UGC','Wallet','Profile','Create Promotion','Create UGC','Add Funds','Checkout']) assert.ok(business.includes(label),label)
  for(const label of ['View Only','View & Sale','Request to Join']) assert.ok(promotions.includes(label),label)
  for(const label of ['Customer Offer','Creator payment','UGC funding required','funded allocation']) assert.ok(ugc.includes(label),label)
  assert.match(ugc,/CustomerOfferEnabled|customerOfferEnabled/)
  assert.match(customer,/UGC_CUSTOMER_OFFER/)
  assert.match(customer,/Show this QR at checkout/)
  for(const label of ['Checkout','Recent Sales','Profile','Scan Customer QR','Enter Manually']) assert.ok(cashier.includes(label),label)
  for(const label of ['Payment \/ transaction reference','Take Photo','Choose Image','Pending Review']) assert.ok(wallet.includes(label),label)
  assert.match(financial,/Creator Sale share is always 0%/)
  assert.match(financial,/UGC Platform deliverable fee/)
  assert.match(styles,/\.account-shell \.workspace-nav\{position:fixed/)
  assert.match(styles,/\.role-cashier \.cashier-tabs \{[\s\S]*position:fixed/)
})
