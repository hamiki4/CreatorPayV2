...
export function MerchantWorkspace(){
 ...
 return <section className="creator-section business-profile" aria-labelledby="business-profile"><h2 id="business-profile">Profile</h2>{error&&<p className="friendly-error" role="alert">{error}</p>}<div className="compact-panel profile-details"><dl><div><dt>Business Name</dt><dd><strong>{profile.tradingName}</strong></dd></div><div><dt>Business Type</dt><dd>{profile.businessType}</dd></div><div><dt>Primary Contact</dt><dd>{profile.primaryContactName}</dd></div><div><dt>Phone</dt><dd>{profile.phoneNumber}</dd></div><div><dt>Email</dt><dd>{profile.email||'No email provided'}</dd></div><div><dt>Address</dt><dd>{profile.businessAddress}</dd></div><div><dt>Primary City</dt><dd>{profile.city}</dd></div><div><dt>Business ID</dt><dd className="business-copyable-id">{profile.publicMerchantId}</dd></div><div><dt>Status</dt><dd><AccountStatusBadge status={profile.effectiveStatus} /></dd></div></dl></div></section>
}
