...
export function AccountChrome({role,name,status='Active',photoUrl,identityMedia,onProfile,onHelp,onSignOut,onManagement,onNavigate,children}:{role:'Customer'|'Creator'|'Business';name?:string;status?:string;photoUrl?:string;identityMedia?:ReactNode;onProfile:()=>void;onHelp:()=>void;onSignOut:()=>void;onManagement?:()=>void;onNavigate?:(target:string,notice:Notice)=>void;children:ReactNode}){
  ...
  return <div className="account-shell">
    <header className="account-header"><div className="account-header-copy"><p className="eyebrow">Weymela</p>{role==='Creator'?<div className="account-identity account-identity--creator">{identityMedia??<ProfileAvatar name={name??role} photoUrl={photoUrl} className="account-identity-avatar" />}<div className="account-identity-text"><h1>{role}</h1><strong>{name??role}</strong>{status&&<span className="account-status"><span className="account-status-dot" aria-hidden="true">●</span><span>{status}</span></span>}</div></div>:<><h1>{role}</h1><p className="account-identity"><span>{name??role}</span>{status&&<span className="account-status">{status}</span>}</p></>}</div><div className="account-actions">
