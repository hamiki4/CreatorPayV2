...
export function AccountChrome({role,name,status='Active',identityMedia,onProfile,onHelp,onSignOut,onManagement,onNavigate,children}:{role:'Customer'|'Creator'|'Business';name?:string;status?:string;identityMedia?:ReactNode;onProfile:()=>void;onHelp:()=>void;onSignOut:()=>void;onManagement?:()=>void;onNavigate?:(target:string,notice:Notice)=>void;children:ReactNode}){
  ...
  return <div className="account-shell">
    <header className="account-header"><div><p className="eyebrow">Weymela</p><h1>{role}</h1><p className="account-identity">{identityMedia&&<span className="account-identity-media">{identityMedia}</span>}<span>{name??role}</span>{status&&<span className="account-status">{status}</span>}</p></div><div className="account-actions">
