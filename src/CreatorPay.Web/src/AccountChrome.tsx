...
export function AccountChrome({role,name,status='Active',photoUrl,onProfile,onHelp,onSignOut,onManagement,onNavigate,children}:{role:'Customer'|'Creator'|'Business';name?:string;status?:string;photoUrl?:string;onProfile:()=>void;onHelp:()=>void;onSignOut:()=>void;onManagement?:()=>void;onNavigate?:(target:string,notice:Notice)=>void;children:ReactNode}){
  ...
  return <div className="account-shell">
    <header className="account-header"><div className="account-header-copy"><p className="eyebrow">Weymela</p>{role==='Creator'?<div className="account-identity account-identity--creator"><ProfileAvatar name={name??role} photoUrl={photoUrl} className="account-identity-avatar" /><div className="account-identity-text"><h1>{role}</h1><strong>{name??role}</strong>{status&&<span className="account-status"><span className="account-status-dot" aria-hidden="true">●</span><span>{status}</span></span>}</div></div>:<><h1>{role}</h1><p className="account-identity"><span>{name??role}</span>{status&&<span className="account-status">{status}</span>}</p></>}</div><div className="account-actions">
      <button type="button" className="icon-button" aria-label="Notifications" aria-expanded={notificationsOpen} onClick={()=>{setNotificationsOpen(!notificationsOpen);setSettingsOpen(false);requestActionableRefresh();if(!notificationsOpen)void refresh()}}><svg aria-hidden="true" viewBox="0 0 24 24"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9M10 21h4"/></svg>{unread>0&&<span className="notification-count">{unread>99?'99+':unread}</span>}</button>
      <button ref={settingsButtonRef} type="button" className="icon-button" aria-label="Settings" aria-expanded={settingsOpen} onClick={()=>{setSettingsOpen(!settingsOpen);setNotificationsOpen(false)}}><svg aria-hidden="true" viewBox="0 0 24 24"><path d="M12 15.5a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7Z"/><path d="M19.4 15a1.7 1.7 0 0 0 .3 1.9l.1.1-2.8 2.8-.1-.1a1.7 1.7 0 0 0-1.9-.3 1.7 1.7 0 0 0-1 1.6v.2h-4V21a1.7 1.7 0 0 0-1-1.6 1.7 1.7 0 0 0-1.9.3l-.1.1L4.2 17l.1-.1a1.7 1.7 0 0 0 .3-1.9A1.7 1.7 0 0 0 3 14H2.8v-4H3a1.7 1.7 0 0 0 1.6-1 1.7 1.7 0 0 0-.3-1.9L4.2 7 7 4.2l.1.1a1.7 1.7 0 0 0 1.9.3A1.7 1.7 0 0 0 10 3V2.8h4V3a1.7 1.7 0 0 0 1 1.6 1.7 1.7 0 0 0 1.9-.3l.1-.1L19.8 7l-.1.1a1.7 1.7 0 0 0-.3 1.9 1.7 1.7 0 0 0 1.6 1h.2v4H21a1.7 1.7 0 0 0-1.6 1Z"/></svg></button>
      {settingsOpen&&<div ref={settingsRef} className="settings-menu" role="menu"><button role="menuitem" onClick={()=>{setSettingsOpen(false);onProfile()}}>Profile</button>{onManagement&&<button role="menuitem" onClick={()=>{setSettingsOpen(false);onManagement()}}>Cashier Management</button>}<button role="menuitem" onClick={()=>{setSettingsOpen(false);void enablePushNotifications().catch(()=>setNotificationError('Notifications could not be enabled.'))}}>Enable device notifications</button><button role="menuitem" onClick={()=>{setSettingsOpen(false);onHelp()}}>Help</button><button role="menuitem" className="signout-action" onClick={()=>{setSettingsOpen(false);onSignOut()}}>Sign out</button></div>}
    </div></header>
    {children}
