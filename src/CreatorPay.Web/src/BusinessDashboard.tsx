...
export function BusinessDashboard({cashiers,checkout,wallet,profile,onSignOut}:{cashiers:ReactNode;checkout:ReactNode;wallet:ReactNode;profile:ReactNode;onSignOut:()=>void}){
 const[tab,setTab]=useState<Tab>('home'),[requestSection,setRequestSection]=useState<RequestSection>('creator'),[items,setItems]=useState<BusinessRelationship[]>([]),[business,setBusiness]=useState<Profile>(),[balance,setBalance]=useState<WalletSummary>(),[metrics,setMetrics]=useState<Metrics>(),[error,setError]=useState('')
 ...
 return <AccountChrome role="Business" name={business?.tradingName} status={business?.effectiveStatus} onProfile={()=>setTab('profile')} onManagement={()=>setTab('cashiers')} onHelp={()=>location.assign('/help')} onSignOut={onSignOut} onNavigate={navigate}>
  <div className="creator-dashboard business-dashboard">
   <RoleNavigation role="Business" label="Business sections" items={[{id:'home',label:'Home',icon:'home',active:tab==='home'||tab==='confirmed-sales'||tab==='wallet'||tab==='cashiers',onSelect:()=>setTab('home')},{id:'find',label:'Creators',icon:'creators',active:tab==='find',onSelect:()=>setTab('find')},{id:'active',label:'Active Ads',icon:'ads',active:tab==='active',onSelect:()=>setTab('active')},{id:'requests',label:'Requests',icon:'requests',active:tab==='requests',onSelect:()=>openRequests('creator')},{id:'checkout',label:'Checkout',icon:'checkout',active:tab==='checkout',onSelect:()=>setTab('checkout')},{id:'profile',label:'Profile',icon:'profile',active:tab==='profile',onSelect:()=>setTab('profile')}]} />
