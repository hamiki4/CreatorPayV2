...
type SessionState = { accessToken: string; refreshToken: string; trustedPhone: string }
let state: SessionState = { accessToken: '', refreshToken: '', trustedPhone: '' }
let hydrated = false
...
export async function hydrateSession() {
  const [accessToken, refreshToken, trustedPhone] = await Promise.all([
    read(keys.accessToken), read(keys.refreshToken), read(keys.trustedPhone),
  ])
  state = { accessToken, refreshToken, trustedPhone }
  hydrated = true
}
