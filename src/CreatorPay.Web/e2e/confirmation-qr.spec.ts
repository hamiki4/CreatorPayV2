...
async function login(page:Page,phone:string){
  ...
  const welcomeSignIn=page.getByRole('button',{name:'Already have an account? Sign In'})
  if(await welcomeSignIn.isVisible())await welcomeSignIn.click()
...
async function cashierCreates(page:Page,key:string,amount:string){
  const device=key.split('-')[1],deviceDigit=device==='desktop'?'1':device==='mobile'?'2':'3'
  const phone=`09117700${key.includes('yes')?'1':'2'}${deviceDigit}`
  await login(page,creatorPhone);await page.getByRole('button',{name:'Settings'}).click();await page.getByRole('menuitem',{name:'Profile'}).click();await expect(page.getByText('Creator ID',{exact:true})).toBeVisible();await expect(page.getByText(creatorCode,{exact:true})).toBeVisible();await expect(page.getByRole('img',{name:'Creator QR code'})).toHaveCount(0);await logout(page)
  await login(page,ownerPhone);await page.getByRole('button',{name:'Settings'}).click();await page.getByRole('menuitem',{name:'Cashier Management'}).click();await page.getByRole('button',{name:'Create Cashier'}).click();await page.getByLabel('Cashier Name').fill('Direct E2E Cashier');await page.getByLabel('Phone Number').fill(phone);await page.getByLabel('Temporary Password',{exact:true}).fill(password);await page.getByLabel('Confirm Temporary Password').fill(password);await expect(page.getByText('Birth Date')).toHaveCount(0);await page.getByRole('button',{name:'Create Cashier'}).click();await expect(page.getByText('Cashier account created.')).toBeVisible();await logout(page)
...
test('Cashier Creator ID purchase waits for Shopper YES and confirms once',async({page,request},testInfo)=>{
  const device=testInfo.project.name==='mobile'?'mobile':testInfo.project.name==='iphone'?'iphone':'desktop',key=`yes-${device}`,amount=device==='mobile'?'511':device==='iphone'?'512':'510',id=await cashierCreates(page,key,amount);await logout(page);await login(page,phones[key]);await page.getByRole('button',{name:/Purchase confirmations/}).click()
  const pending=page.locator('.pending-confirmations article').filter({hasText:'Active E2E Business'});await expect(pending).toContainText(`${amount}.00 ETB`);await expect(pending).toContainText('Is this your purchase?');await pending.getByRole('button',{name:"YES, IT'S ME",exact:true}).click();await expect(page.getByText('Purchase confirmed.')).toBeVisible();await expect(pending).toHaveCount(0)
  ...
  await logout(page);const cashierPhone=`091177001${device==='desktop'?'1':device==='mobile'?'2':'3'}`;await login(page,cashierPhone);await page.getByRole('button',{name:'Recent Transactions'}).click();await expect(page.locator('.cashier-transaction-row.headings')).toContainText('Amount');await expect(page.locator('.cashier-transaction-row.headings')).toContainText('Status');await expect(page.locator('.cashier-transaction-row.headings')).toContainText('Date');await expect(page.locator('.cashier-transaction-row.headings')).toContainText('Time');await expect(page.locator('.cashier-transaction-row').nth(1)).not.toContainText(/CP-|[0-9a-f]{8}-[0-9a-f]{4}/i);expect(await page.evaluate(()=>document.documentElement.scrollWidth<=document.documentElement.clientWidth)).toBeTruthy()
})

test('Cashier Creator ID purchase waits for Shopper NO with no settlement',async({page},testInfo)=>{
  const device=testInfo.project.name==='mobile'?'mobile':testInfo.project.name==='iphone'?'iphone':'desktop',key=`no-${device}`,amount=device==='mobile'?'521':device==='iphone'?'522':'520';await cashierCreates(page,key,amount);await logout(page);await login(page,phones[key]);await page.getByRole('button',{name:/Purchase confirmations/}).click()
  const pending=page.locator('.pending-confirmations article').filter({hasText:'Active E2E Business'});await expect(pending).toContainText(`${amount}.00 ETB`);await pending.getByRole('button',{name:"NO, IT'S NOT ME",exact:true}).click();await expect(page.getByText('Purchase rejected.')).toBeVisible();await expect(pending).toHaveCount(0)
