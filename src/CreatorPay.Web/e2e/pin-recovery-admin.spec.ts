...
const birth={day:'2',month:'1',year:'1990'}
...
async function recover(page:any,request:any,phone:string){
  const seeded=await request.post(`${api}/api/v1/e2e/locked-pin-user`,{data:{phoneNumber:phone,password,pin:oldPin,birthDate:'1990-01-02'}});expect(seeded.status()).toBe(200)
  await page.goto('/');await page.evaluate(value=>localStorage.setItem('weymela_trusted_phone',value),phone);await page.reload()
  await page.getByRole('button',{name:'Forgot PIN'}).click();await page.getByLabel('Day').fill(birth.day);await page.getByLabel('Month').fill(birth.month);await page.getByLabel('Year').fill(birth.year);await page.getByRole('button',{name:'Continue'}).click()
...
test('birth-date recovery rejects the old PIN',async({page,request},testInfo)=>{
  const phone=testInfo.project.name==='mobile'?'+251977210002':testInfo.project.name==='iphone'?'+251977210003':'+251977210001';await recover(page,request,phone);await pin(page,oldPin);await expect(page.getByRole('alert')).toContainText('Invalid phone number or PIN.')
})

test('locked user recovers by birth date and signs in with the new PIN',async({page,request},testInfo)=>{
  const phone=testInfo.project.name==='mobile'?'+251977200002':testInfo.project.name==='iphone'?'+251977200003':'+251977200001'
  const seeded=await request.post(`${api}/api/v1/e2e/locked-pin-user`,{data:{phoneNumber:phone,password,pin:oldPin,birthDate:'1990-01-02'}})
  ...
  await page.getByLabel('Day').fill(birth.day);await page.getByLabel('Month').fill(birth.month);await page.getByLabel('Year').fill(birth.year)
