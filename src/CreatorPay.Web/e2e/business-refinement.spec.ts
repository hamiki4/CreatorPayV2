...
for(const viewport of viewports){
  test(`Business reference dashboard and routes at ${viewport.name}`,async({page})=>{
    ...
    await expect(cards.locator('.business-dashboard-label')).toHaveText(['Active Ads','Pending Requests','Confirmed Sales','Wallet Balance'])
    ...
    await expect(nav.getByRole('button')).toHaveText(['Creators','Active Ads','Requests','Checkout','Profile'])
    await expect(nav.getByRole('button',{name:'Creators',exact:true})).toHaveCSS('color','rgb(37, 99, 235)')
    ...
    await page.reload()
    await expect(page.getByRole('heading',{name:'Dashboard',exact:true})).toBeVisible()
    await page.locator('.business-dashboard-cards > button').filter({hasText:'Pending Requests'}).click()
