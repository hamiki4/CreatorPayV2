...
for(const viewport of viewports){
  test(`Business reference dashboard and routes at ${viewport.name}`,async({page})=>{
    ...
    const firstCreatorId=await topCreators.first().locator('.business-top-creator-heading > div > span').innerText()
    ...
    await expect(page.getByLabel('Creator name or public ID')).toHaveValue(firstCreatorId)
