...
test('Shopper password confirmation uses submitted values and sends one request',async({page},testInfo)=>{
  const phone=testInfo.project.name==='mobile'?'0911779012':testInfo.project.name==='iphone'?'0911779013':'0911779011',requests:string[]=[]
  ...
  const response=page.waitForResponse(value=>value.url().includes('/api/v1/customers/register')&&value.request().method()==='POST');await form.evaluate((node:HTMLFormElement)=>{node.requestSubmit();node.requestSubmit()});expect((await response).ok()).toBeTruthy();await expect(page.getByRole('status')).toContainText('Shopper account created. You can sign in now.');await expect(page.getByRole('heading',{name:'Verify your phone'})).toHaveCount(0);expect(requests).toHaveLength(1);const body=JSON.parse(requests[0]);expect(body.email).toBe('');expect(body.password).toBe('Test@1234');expect(body.confirmation).toBe('Test@1234')
})
