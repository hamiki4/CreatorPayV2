...
async function cashierCreates(page:Page,key:string,amount:string){
  ...
  await login(page,phone);await expect(page.getByRole('heading',{name:'Cashier Dashboard'})).toBeVisible();await expect(page.getByRole('button',{name:'New Purchase'})).toBeVisible();await expect(page.getByLabel('Creator ID')).toHaveValue('');await expect(page.getByLabel('Creator ID')).toHaveAttribute('placeholder','Creator ID');await expect(page.getByLabel('Customer Phone Number')).toHaveValue('');await expect(page.getByLabel('Customer Phone Number')).toHaveAttribute('placeholder','Customer Phone Number');await expect(page.getByLabel('Purchase Amount (ETB)')).toHaveValue('');await expect(page.getByLabel('Purchase Amount (ETB)')).toHaveAttribute('placeholder','Purchase Amount');await page.getByLabel('Creator ID').fill(creatorCode);await page.getByLabel('Customer Phone Number').fill(phones[key]);await page.getByLabel('Purchase Amount (ETB)').fill(amount);page.once('dialog',dialog=>dialog.accept());const validation=page.waitForResponse(r=>r.url().includes('/api/v1/cashier/checkouts/validate-creator')&&r.request().method()==='POST');const response=page.waitForResponse(r=>r.url().includes('/api/v1/cashier/checkouts/by-creator')&&r.request().method()==='POST');await page.getByRole('button',{name:'Submit Purchase'}).click();expect((await validation).ok()).toBeTruthy();const body=await (await response).json();expect(body.code).toBe('awaiting_shopper_confirmation');await expect(page.getByText('Purchase submitted — awaiting Shopper confirmation.',{exact:true})).toBeVisible();expect(await page.evaluate(()=>document.documentElement.scrollWidth<=document.documentElement.clientWidth)).toBeTruthy();return body.checkout.id as string
}
