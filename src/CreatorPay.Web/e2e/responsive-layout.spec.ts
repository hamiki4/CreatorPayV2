...
for (const width of widths) {
  test(`mobile customer, creator, and business layouts stay within ${width}px`, async ({ page }) => {
    ...
    await expect(creatorNav.getByRole('button')).toHaveText(['Find Businesses', 'Active Ads', 'Requests', 'Confirmed Sales', 'Payout', 'Profile'])
    ...
    await expect(creatorNav.getByRole('button', { name: 'Confirmed Sales', exact: true })).toHaveAttribute('aria-current', 'page')
