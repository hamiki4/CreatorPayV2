...
for (const {width,height} of mobileViewports) {
  test(`mobile customer, creator, and business layouts stay within ${width}x${height}`, async ({ page }) => {
    ...
    const titleBox = await page.locator('.role-creator .account-header h1').boundingBox()
    const identityBox = await page.locator('.role-creator .account-identity--creator').boundingBox()
    ...
    await expect(businessNav.getByRole('button')).toHaveText(['Home', 'Creators', 'Active Ads', 'Requests', 'Checkout', 'Profile'])
    await expect(businessNav.getByRole('button', { name: 'Home', exact: true })).toHaveAttribute('aria-current', 'page')
    ...
    await expect(businessNav.getByRole('button', { name: 'Home', exact: true })).not.toHaveAttribute('aria-current', 'page')
    ...
    await businessNav.getByRole('button', { name: 'Home', exact: true }).click()
