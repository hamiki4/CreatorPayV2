...
for (const width of widths) {
  test(`mobile customer, creator, and business layouts stay within ${width}px`, async ({ page }) => {
    ...
    await expect(creatorNav.getByRole('button')).toHaveText(['Find', 'Active Ads', 'Requests', 'Payout', 'Profile'])
    await expect(creatorNav.getByRole('button', { name: 'Find', exact: true })).toHaveAttribute('aria-current', 'page')
    ...
    await expect(creatorNav.getByRole('button', { name: 'Find', exact: true })).not.toHaveAttribute('aria-current', 'page')
