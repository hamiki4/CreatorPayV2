...
public sealed record CreatorProfileResponse(Guid CreatorId, string PublicCreatorId, string CreatorCode, string FirstName, string LastName, string DisplayName, string PhoneNumber, string Email, bool IsEmailVerified, bool IsPhoneVerified, AccountStatus AccountStatus, CreatorStatus CreatorStatus, string EffectiveStatus, string EffectiveStatusReason, ProfileImageMetadata? ProfileImage, string NextStep, string PreferredLanguage, string City, string? Zone, string Biography, string ContentCategories, IReadOnlyList<SocialProfileResponse> SocialProfiles, string? PreferredPayoutChannel, string? PreferredPayoutAccountIdentifier);
