...
public sealed record StaffInvitationRequest(string FirstName,string LastName,string Email,string PhoneNumber,IReadOnlyList<Guid>? LocationIds);
...
public sealed record StaffResponse(Guid Id,string FirstName,string LastName,string Email,string PhoneNumber,bool IsActive,IReadOnlyList<StaffLocationResponse> Locations);
public sealed record StaffLocationResponse(Guid Id,string Name,bool IsActive,bool IsPrimary);
public sealed record InvitationResponse(Guid InvitationId,string Email,UserRole Role,DateTime ExpiresAtUtc,string? DevelopmentToken);
