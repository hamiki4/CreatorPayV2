...
public static class E2eSeedEndpoints
{
    ...
    private static async Task<IResult> SeedAsync(E2eSeedRequest request, ApplicationDbContext db, IPasswordHasher passwords, IQrTokenService qrTokens,CreatorQrUrlBuilder qrUrls, CancellationToken ct)
    {
        ...
        var pilotFinancialSetting=await db.PlatformFinancialSettings.SingleAsync(x=>x.CurrencyCode=="ETB",ct);pilotFinancialSetting.MinimumBusinessWalletBalance=0m;pilotFinancialSetting.ChangedAtUtc=DateTime.UtcNow;
        ...
        var secondPartnership=new MerchantCreatorPartnership{Id=Id("40000000-0000-0000-0000-000000000002"),MerchantId=merchant.Id,CreatorId=secondCreator.Id,RequestedAtUtc=now,CreatedAtUtc=now};secondPartnership.Approve(now,user.Id);
        ...
        db.AddRange(shopper,user,new CustomerWallet{Id=Id("10000000-0000-0000-0000-000000000003"),CustomerId=shopper.Id,CurrencyCode="ETB",CreatedAtUtc=now},merchant,ownerUser,inactiveMerchant,location,cashier,cashierUser,cashierAssignment,wallet,active,creatorQr,activeCreatorUser,secondCreator,secondCreatorUser,secondQr,secondPartnership,secondOffer,adminUser,new CreatorBalanceAccount{Id=Id("30000000-0000-0000-0000-000000000014"),CreatorId=active.Id,CurrencyCode="ETB",CreatedAtUtc=now},new CreatorBalanceAccount{Id=Guid.NewGuid(),CreatorId=secondCreator.Id,CurrencyCode="ETB",CreatedAtUtc=now},noOffer,noOfferQr,suspended,partnership,plan,rule,version,offer,commissionAssignment,platformAssignment,expired,expiredCheckout,new MerchantTrialCredit{Id=Id("20000000-0000-0000-0000-000000000004"),MerchantId=merchant.Id,CreatedAtUtc=now});db.AddRange(workflowEntities);db.AddRange(confirmationShoppers);db.AddRange(confirmationAccounts);
        await db.SaveChangesAsync(ct);
