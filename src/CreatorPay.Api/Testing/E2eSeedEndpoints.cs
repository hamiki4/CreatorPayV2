...
public static class E2eSeedEndpoints
{
    ...
    private static async Task<IResult> SeedAsync(E2eSeedRequest request, ApplicationDbContext db, IPasswordHasher passwords, IQrTokenService qrTokens, CreatorQrUrlBuilder qrUrls, IWebHostEnvironment environment, CancellationToken ct)
    {
        ...
        var location = new MerchantLocation { Id = Id("20000000-0000-0000-0000-000000000003"), MerchantId = merchant.Id, Name = "E2E Checkout", AddressLine1 = "Synthetic", City = "Addis Ababa", CountryCode = "ET", TimeZoneId = "Africa/Addis_Ababa", Latitude = 9.0300, Longitude = 38.7400, IsActive = true, CreatedAtUtc = now };
        ...
        var approvedMetricMerchant = Merchant(Id("22000000-0000-0000-0000-000000000001"), "Approved Metric Business", MerchantStatus.Active, now); approvedMetricMerchant.PublicMerchantId = "MER-METRIC-APPROVED";
        var pendingMetricMerchant = Merchant(Id("22000000-0000-0000-0000-000000000002"), "Pending Metric Business", MerchantStatus.Active, now); pendingMetricMerchant.PublicMerchantId = "MER-METRIC-PENDING";
        var rejectedMetricMerchant = Merchant(Id("22000000-0000-0000-0000-000000000003"), "Rejected Metric Business", MerchantStatus.Active, now); rejectedMetricMerchant.PublicMerchantId = "MER-METRIC-REJECTED";
        var approvedMetricPartnership = new MerchantCreatorPartnership { Id = Id("42000000-0000-0000-0000-000000000001"), MerchantId = approvedMetricMerchant.Id, CreatorId = active.Id, RequestedAtUtc = now.AddHours(-3), CreatedAtUtc = now.AddHours(-3) }; approvedMetricPartnership.Approve(now.AddHours(-2), user.Id);
        var pendingMetricPartnership = new MerchantCreatorPartnership { Id = Id("42000000-0000-0000-0000-000000000002"), MerchantId = pendingMetricMerchant.Id, CreatorId = active.Id, RequestedAtUtc = now.AddHours(-2), CreatedAtUtc = now.AddHours(-2) };
        var rejectedMetricPartnership = new MerchantCreatorPartnership { Id = Id("42000000-0000-0000-0000-000000000003"), MerchantId = rejectedMetricMerchant.Id, CreatorId = active.Id, RequestedAtUtc = now.AddHours(-4), CreatedAtUtc = now.AddHours(-4) }; rejectedMetricPartnership.Reject(now.AddHours(-3), user.Id, "Synthetic regression state");
