...
public static class E2eSeedEndpoints
{
    ...
    private static async Task<IResult> SeedAsync(E2eSeedRequest request, ApplicationDbContext db, IPasswordHasher passwords, CancellationToken ct)
    {
        ...
        var offer=Campaign(Id("60000000-0000-0000-0000-000000000001"),"CMP-E2E-ACTIVE",active.Id,merchant.Id,partnership.Id,version.Id,"E2EACTIVE",now.AddMinutes(-1),30,user.Id);
        var expired=Campaign(Id("60000000-0000-0000-0000-000000000002"),"CMP-E2E-EXPIRED",active.Id,merchant.Id,partnership.Id,version.Id,"E2EEXPIRED",now.AddDays(-2),1,user.Id); expired.ExpireIfDue(now);
        ...
        db.AddRange(shopper,user,new CustomerWallet{Id=Id("10000000-0000-0000-0000-000000000003"),CustomerId=shopper.Id,CurrencyCode="ETB",CreatedAtUtc=now},merchant,ownerUser,inactiveMerchant,location,cashier,cashierUser,cashierAssignment,wallet,active,new CreatorBalanceAccount{Id=Id("30000000-0000-0000-0000-000000000014"),CreatorId=active.Id,CurrencyCode="ETB",CreatedAtUtc=now},noOffer,suspended,partnership,plan,rule,version,offer,commissionAssignment,expired,expiredCheckout,new MerchantTrialCredit{Id=Id("20000000-0000-0000-0000-000000000004"),MerchantId=merchant.Id,CreatedAtUtc=now});
        await db.SaveChangesAsync(ct);
        return Results.Ok(new{shopperEmail=user.Email,cashierEmail=cashier.Email,locationId=location.Id,expiredCheckoutQr=$"creatorpay:checkout:{expiredCheckout.PublicCheckoutId}:{expiredRaw}",creatorName=active.DisplayName,activeOfferCode=offer.CampaignCode,expiredOfferCode=expired.CampaignCode,offerQrId});
    }
    ...
    private static CreatorMerchantCampaign Campaign(Guid id,string publicId,Guid creator,Guid merchant,Guid partnership,Guid version,string code,DateTime start,int days,Guid actor){var x=new CreatorMerchantCampaign{Id=id,PublicCampaignId=publicId,CreatorId=creator,MerchantId=merchant,MerchantCreatorPartnershipId=partnership,CreatedAtUtc=start};x.Approve(days,null,version,actor,code,"E2E Active Offer\nSynthetic public description",start);x.QrCode=new(){Id=Guid.NewGuid(),CampaignId=id,PublicQrId=$"CQR-{code}",TokenHash=Convert.ToHexString(System.Security.Cryptography.SHA256.HashData(System.Text.Encoding.UTF8.GetBytes(code))),IssuedAtUtc=start,CreatedAtUtc=start};x.Start(start);x.QrCode.Activate(start,x.ExpiresAtUtc!.Value);return x;}
}
