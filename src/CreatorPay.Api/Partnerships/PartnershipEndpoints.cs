...
public static class PartnershipEndpoints
{
    ...
    private static async Task<IResult> CreatorInvitationDecision(Guid id, bool accept, ICurrentUserService u, ApplicationDbContext db, ICommissionEngine commissions, IOptions<CampaignOptions> campaignOptions, INotificationService notifications, HttpContext h, CancellationToken ct)
    {
        ...
        var now = DateTime.UtcNow; var old = p.Status; var promotionActive = false;
        if (accept)
        {
            ...
            if (await IsActivationReady(p, db, campaignOptions.Value, ct)) { await EnsurePromotion(p, u.UserAccountId.Value, db, commissions, campaignOptions.Value, now, ct); promotionActive = true; }
        }
        ...
        await db.SaveChangesAsync(ct); return Results.Ok(Item(p) with { InitiatedBy = "Business", PromotionActive = promotionActive, RelationshipState = promotionActive ? "Active" : accept ? "ActivationRequired" : "Declined", ActivationRequired = accept && !promotionActive });
    }
    ...
    private static async Task<IResult> MerchantAdd(MerchantAddCreatorRequest request, ICurrentUserService u, ApplicationDbContext db, ICommissionEngine commissions, IOptions<CampaignOptions> campaignOptions, HttpContext h, CancellationToken ct)
    {
        ...
        if (await IsActivationReady(p, db, campaignOptions.Value, ct)) { await EnsurePromotion(p, u.UserAccountId.Value, db, commissions, campaignOptions.Value, now, ct); promotionActive = true; }
        await db.SaveChangesAsync(ct); return Results.Created($"/api/v1/merchant/partnerships/{p.Id}", Item(p, await db.Merchants.FindAsync([u.MerchantId.Value], ct) ?? new(), creator) with { PromotionActive = promotionActive, RelationshipState = promotionActive ? "Active" : "ActivationRequired", ActivationRequired = !promotionActive });
    }
    ...
    private static async Task<IResult> Transition(Guid id, PartnershipStatus target, string? reason, ICurrentUserService u, ApplicationDbContext db, ICommissionEngine commissions, IOptions<CampaignOptions> campaignOptions, INotificationService notifications, HttpContext h, CancellationToken ct, bool reactivationOnly = false)
    {
        ...
        await db.SaveChangesAsync(ct); return Results.Ok(Item(p) with { PromotionActive = promotionActive, RelationshipState = target == PartnershipStatus.Approved ? promotionActive ? "Active" : "ActivationRequired" : State(p.Status), ActivationRequired = target == PartnershipStatus.Approved && !promotionActive });
    }
    ...
    private static async Task<IResult> ReconcileReadiness(Guid id, ICurrentUserService u, ApplicationDbContext db, ICommissionEngine commissions, IOptions<CampaignOptions> campaignOptions, CancellationToken ct)
    {
        ...
        if (!await IsActivationReady(p, db, campaignOptions.Value, ct)) return Results.Ok(Item(p) with { RelationshipState = "ActivationRequired", ActivationRequired = true });
        var now = DateTime.UtcNow;
        try { await EnsurePromotion(p, u.UserAccountId!.Value, db, commissions, campaignOptions.Value, now, ct); }
        catch (CommissionConfigurationException ex) { db.ChangeTracker.Clear(); return Problem(409, $"Advertising cannot be activated until Platform commission settings are configured. {ex.Message}"); }
    ...
    private static async Task<bool> IsActivationReady(MerchantCreatorPartnership partnership, ApplicationDbContext db, CampaignOptions options, CancellationToken ct)
    {
        ...
        if (!merchantReady || !await db.Creators.AsNoTracking().AnyAsync(x => x.Id == partnership.CreatorId && x.Status == CreatorStatus.Active, ct)) return false;
