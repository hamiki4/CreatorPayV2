...
public static class PartnershipEndpoints
{
    public static IEndpointRouteBuilder MapPartnershipEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        merchant.MapPost("/partnerships/{id:guid}/approve", MerchantApprove);
    ...
    private static async Task<IResult> CreatorInvitationDecision(Guid id, bool accept, ICurrentUserService u, ApplicationDbContext db, ICommissionEngine commissions, IOptions<CampaignOptions> campaignOptions, INotificationService notifications, HttpContext h, CancellationToken ct)
    {
        ...
        var now = DateTime.UtcNow; var old = p.Status; if (accept) { p.Approve(now, u.UserAccountId!.Value, p.StartDateUtc, p.EndDateUtc); try { await EnsurePromotion(p, u.UserAccountId.Value, db, commissions, campaignOptions.Value, now, ct); } catch (CommissionConfigurationException ex) { db.ChangeTracker.Clear(); return Problem(409, $"Advertising cannot be activated until Platform commission settings are configured. {ex.Message}"); } } else p.Reject(now, u.UserAccountId!.Value, "Declined by creator");
        ...
        await db.SaveChangesAsync(ct); return Results.Ok(Item(p) with { InitiatedBy = "Business", PromotionActive = accept });
    }
    ...
    private static async Task<IResult> MerchantApprove(Guid id, PartnershipReasonRequest r, ICurrentUserService u, ApplicationDbContext db, ICommissionEngine commissions, IOptions<CampaignOptions> campaignOptions, INotificationService notifications, HttpContext h, CancellationToken ct)
    {
        var status = await db.MerchantCreatorPartnerships.AsNoTracking().Where(x => x.Id == id && x.MerchantId == u.MerchantId).Select(x => (PartnershipStatus?)x.Status).SingleOrDefaultAsync(ct);
        if (!status.HasValue) return NotFound();
        if (status.Value != PartnershipStatus.Pending) return Problem(409, "Only a pending Creator request can be approved.");
        return await Transition(id, PartnershipStatus.Approved, r.Reason, u, db, commissions, campaignOptions, notifications, h, ct);
    }
