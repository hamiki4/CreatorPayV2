...
public static class MerchantEndpoints
{
    public static IEndpointRouteBuilder MapMerchantEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        admin.MapPost("/request-correction", async (MerchantDecisionRequest r, ICurrentUserService u, IMerchantService s, ExternalIntegrationService integration, CancellationToken ct) =>
        { var result = await s.RequestCorrectionAsync(r.MerchantId, u.UserAccountId!.Value, r.Reason, ct); var synchronized = await integration.SynchronizeLifecycleAsync(UserRole.MerchantAdmin, r.MerchantId, "CORRECTION_REQUESTED", ct); return synchronized && !result.Succeeded ? Results.Ok(new { succeeded = true }) : ToHttp(result); });
