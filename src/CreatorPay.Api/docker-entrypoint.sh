...
keyring_root="${DataProtection__KeyRingPath:-$proof_root/data-protection-keys}"
...
if [ "$(id -u)" = "0" ]; then
  ...
  install -d -m 0750 -o "$app_uid" -g "$app_uid" "$keyring_root"
  chown -R "$app_uid:$app_uid" "$keyring_root"
  chmod 0750 "$keyring_root"
