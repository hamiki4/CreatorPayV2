...
public static class DiscoveryEndpoints
{
    public static IEndpointRouteBuilder MapDiscoveryEndpoints(this IEndpointRouteBuilder e)
    {
        ...
        shopper.MapGet("/", (string? q, string? category, double? latitude, double? longitude, IDiscoveryService s, CancellationToken ct) => Run(() => s.SearchShopperBusinessesAsync(q, category, latitude, longitude, ct)));
        shopper.MapGet("/{id:guid}", (Guid id, IDiscoveryService s, CancellationToken ct) => Run(() => s.GetShopperBusinessAsync(id, ct)));
        e.MapGet("/api/v1/customer/discovery/advertising", (string? q, string? businessType, double? latitude, double? longitude, IDiscoveryService s, CancellationToken ct) => Run(() => s.SearchShopperAdvertisingAsync(q, businessType, latitude, longitude, ct))).RequireAuthorization("CustomerOnly");
