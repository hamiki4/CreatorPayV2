...
public sealed class CreatorVerificationTests
{
    ...
    private static CreatorService Service(FakeCreatorStore store) => new(store, new NoopPasswords(), new TestTokens(), new TestClock(), new NoopProvider(), new PasswordPolicyValidator(Options.Create(new PasswordOptions())), Options.Create(new CreatorVerificationOptions()));
    ...
    private sealed class FakeCreatorStore(UserAccount user, Creator creator, params CreatorVerificationToken[] tokens) : ICreatorStore
    {
        ...
        public Task InvalidateTokensAsync(Guid userId, string purpose, DateTime usedAtUtc, CancellationToken ct) => Task.CompletedTask; public void Add(UserAccount item) { } public void Add(Creator item) { } public void Add(CreatorVerificationToken item) { } public void Add(CreatorAuditEvent item) { } public void Add(CreatorSocialProfile item) { } public void RemoveSocialProfiles(IEnumerable<CreatorSocialProfile> profiles) { }
    }
}
